...
export const NewProject = () => {
  ...
  const startFakeUpload = (f: ProjectFile) => {
    setFile(f);
    setUploading(true);
    setProgress(0);
  };
  ...
  const canNext =
    step === 1 ? Boolean(file) && !uploading : step === 2 ? Boolean(name.trim() && prompt.trim()) : true;
  ...
  return (
    <div className="page" style={{ maxWidth: 880 }}>
      ...
      <div className="card" style={{ padding: 28, marginTop: 24 }}>
        ...
        {step === 3 && <Step3 file={file} name={name} prompt={prompt} />}
      </div>
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 12.5, color: "var(--fg-muted)", display: "flex", gap: 12 }}>
              ...
              <span>•</span>
              <span>~{Math.ceil(sizeNum * 2.5)} chunks</span>
            </div>
          </div>
          {!uploading && (
          ...
          <div style={{ marginTop: 16 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8, fontSize: 13 }}>
              <span style={{ color: "var(--fg-muted)" }}>
                Uploading chunks ·{" "}
                <span className="mono">{Math.floor((sizeNum * 2.5 * progress) / 100)}</span> /{" "}
                <span className="mono">{Math.ceil(sizeNum * 2.5)}</span>
              </span>
...
const Step3 = ({ file, name, prompt }: { file: ProjectFile | null; name: string; prompt: string }) => (
  <div>
    <div
      style={{
        fontSize: 11,
    ...
    >
      ...
      <div style={{ fontSize: 13 }}>
        ...
        <div style={{ color: "var(--fg-muted)" }}>
          Transcribe → Cut → Render → Package metadata. You can leave this page; we&apos;ll keep working in the background.
        </div>
      </div>
    </div>
  </div>
