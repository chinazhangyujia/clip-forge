      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, marginBottom: 28, flexWrap: 'wrap' }}>
        <div style={{ minWidth: 0, flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}>
            <Link to="/" className="btn btn-ghost btn-sm" style={{ paddingLeft: 6, marginLeft: -6, color: 'var(--fg-muted)' }}>
          ...
          {editingName ? (
            ...
            <h1
              ...
              style={{ cursor: 'text', padding: '4px 8px', margin: '0 0 0 -8px', borderRadius: 6, transition: 'background .12s', display: 'inline-flex', alignItems: 'center', gap: 8 }}
              onMouseEnter={(e) => e.currentTarget.style.background = 'var(--bg-sunken)'}
              onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}
            >
              {project.name}
              <span style={{ color: 'var(--fg-faint)', opacity: .5 }}><Icon name="edit" size={14}/></span>
            </h1>
          )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr', gap: 24, alignItems: 'flex-start' }}>
        ...
        <aside style={{ position: 'sticky', top: 80, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <PromptCard
            ...
            onEditPrompt={() => setEditingPrompt(true)}
            onSavePrompt={savePrompt}
            onCancelPrompt={() => { setPromptDraft(project.prompt || ''); setEditingPrompt(false); }}
          />
        ...
        <main>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <div>
              ...
              <p style={{ margin: '4px 0 0', fontSize: 13, color: 'var(--fg-muted)' }}>
                {clips.length === 0 ? 'No clips yet — generated when the pipeline finishes.' : `${clips.length} clips generated from this source`}
              </p>
            </div>
            {clips.length > 0 && (
              <div style={{ display: 'flex', gap: 8 }}>
          ...
          {clips.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 14 }}>
              {clips.map(c => (
                <ClipCard key={c.id} clip={c} onClick={() => navigate(`/projects/${project.id}/clips/${c.id}`)}/>
              ))}
            </div>
          )}
          ...
          <div style={{ marginTop: 32 }}>
            ...
            >
              <span style={{ transform: workingOpen ? 'rotate(90deg)' : 'rotate(0deg)', transition: 'transform .15s', display: 'inline-flex', color: 'var(--fg-faint)' }}>
  ...
  <div className="card" style={{ overflow: 'hidden' }}>
    ...
    <div style={{ padding: '10px 14px', borderTop: '1px solid var(--border)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '4px 0' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div className="mono" style={{ fontSize: 11.5, color: 'var(--fg)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
    ...
    >
      ...
      <span style={{
        ...
        color: 'var(--fg-faint)', fontWeight: 500, letterSpacing: 0.04,
      }}>SOON</span>
    </button>
  </div>
  );
...
const PromptCard = ({ project, editingPrompt, promptDraft, setPromptDraft, onEditPrompt, onSavePrompt, onCancelPrompt }) => (
  <div className="card" style={{ padding: 16 }}>
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
      ...
      {!editingPrompt && (
        <button className="btn-ghost" onClick={onEditPrompt} style={{ fontSize: 11.5, color: 'var(--fg-muted)', padding: '2px 6px', borderRadius: 4 }}>
          Edit
        </button>
      )}
    </div>
    {editingPrompt ? (
      <div>
        ...
        <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
          <button className="btn btn-sm btn-primary" onClick={onSavePrompt}>Save</button>
          <button className="btn btn-sm" onClick={onCancelPrompt}>Cancel</button>
        </div>
      </div>
    ) : (
      <div style={{
        fontSize: 12.5, color: 'var(--fg-muted)', lineHeight: 1.55,
        background: 'var(--bg-sunken)', padding: 10,
...
const PipelineCard = ({ pipeline }) => {
  ...
  return (
    <div className="card" style={{ padding: 16 }}>
      <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: 0.06, textTransform: 'uppercase', color: 'var(--fg-faint)', marginBottom: 16 }}>
        Pipeline status
      </div>
      <div style={{ position: 'relative' }}>
        {/* Vertical track */}
        <div style={{
          position: 'absolute', left: 11, top: 11, bottom: 11,
          width: 2,
...
const PipelineStage = ({ stage, state, last }) => {
  const stateConfig = {
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: 12 }}>
        <div style={{ fontSize: 13, fontWeight: 500, lineHeight: 1.35, marginBottom: 8, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', minHeight: 35 }}>
          {clip.title}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 11, color: 'var(--fg-muted)', marginBottom: 10 }}>
...
const VariantChip = ({ active, label }) => (
  <span style={{
  ...
  return (
    <div style={{
      ...
      <div className="mono" style={{
        ...
        fontSize: 10, color: 'oklch(1 0 0 / 0.85)',
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ marginLeft: 2 }}><Icon name="play" size={14}/></span>
        </div>
      </div>
      {/* hover scrub line */}
      <div style={{
        position: 'absolute', bottom: 0, left: 0, right: 0,
        height: 2, background: 'oklch(1 0 0 / 0.2)',
      }}>
        <div style={{
          height: '100%',
          width: hover ? '70%' : '12%',
  ...
  return (
    <div className="card" style={{ padding: 48, textAlign: 'center' }}>
      ...
      <div style={{ color: 'var(--fg-muted)', fontSize: 13 }}>
        {isFailed ? 'Something went wrong during cutting. Re-run the pipeline to try again.' :
         isProcessing ? 'Clips will appear here as soon as the pipeline finishes.' :
...
const WorkingDir = ({ project, clips }) => {
  const files = [
    ...
    ...clips.slice(0, 3).map(c => ({ name: `clips/${c.id}.mp4`, size: `${(8 + Math.random() * 14).toFixed(1)} MB`, kind: 'video' })),
    ...(clips.length > 3 ? [{ name: `clips/… +${clips.length - 3} more`, size: '', kind: 'meta' }] : []),
    ...clips.slice(0, 2).filter(c => c.variants.length > 1).map(c => ({ name: `processed/${c.id}_captions.mp4`, size: `${(12 + Math.random() * 16).toFixed(1)} MB`, kind: 'video' })),
  ];
    ...
    <div style={{
      ...
      {files.map((f, i) => (
        <div key={i} style={{
          ...
          {f.kind !== 'meta' && (
            <button className="btn-ghost" style={{ padding: 4, color: 'var(--fg-faint)', borderRadius: 4 }} title="Download">
              <Icon name="download" size={13}/>
            </button>
          )}
        </div>
